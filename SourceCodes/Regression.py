...
class KRR (Regression):
    ...
    # INIT
    def __init__ (self, params, kernel_func, alpha = 0.01, beta = 0, isModified = True):
        """ 
            Inputs:
    ...
    def gaussian (self, x, y):
        ...
        x = np.asarray(x).flatten()
        y = np.asarray(y).flatten()        
        k = np.exp(-1*np.linalg.norm(x-y)/(2*sigma**2))
        return k.tolist()
    ...
    def modified_gaussian (self, x, y):
        ...
        x = np.asarray(x).flatten()
        y = np.asarray(y).flatten()        
        k = np.exp(-1*np.linalg.norm(x-y)/(2*sigma**2)) + offset
        return k.tolist()
    ...
    def fit_scott (self, X, y):
        ...
        # Get the size of the kernel matrix
        id_size = len(kernel)
        row, col = np.asarray(kernel).shape
        # Find the trained weights using the analytical, closed-form expression       
        self.weights = np.linalg.inv(kernel + self.alpha*np.identity(id_size)- self.beta*(np.ones(row)@kernel@np.ones(col))*np.eye(id_size))@y
        # Update the model to know that it has been fit
        self.isFit = True
        # Save the training data which will be needed in the predict function
        self.X_train = X        
